...
are 32 test classes (`src/Test*.java`); list them yourself with a glob rather
than trusting this count if files have been added/removed since:

```
"/c/Program Files/Eclipse Adoptium/jdk-21.0.11.10-hotspot/bin/java.exe" -cp "build;lib/*" org.junit.runner.JUnitCore TestAchievement TestAchievementEngine TestAchievementsView TestAchievementToast TestAIMedium TestAIMediumCardPlay TestBetLegality TestBetStepper TestCard TestDeck TestGame TestGameOverBanner TestGameStateCodec TestGameStateStore TestHand TestHandler TestHumanBet TestHumanCheckpointCallback TestHumanIllegalReason TestHumanNextTrick TestHumanPlayCard TestIllegalPlayFeedback TestJson TestKeyInput TestNextTrickPrompt TestPlayer TestPlayerArchetypeRegistry TestRound TestRulesView TestSaveData TestSaveStore TestStartScreen TestTrick TestWindowSizing
```
