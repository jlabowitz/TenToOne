...
|---|------|--------|------|----------------|
| 1 | AI v2: opponent-modeling & endgame heuristics | in progress | M | `game-designer` → `senior-developer` |
| 2 | Game log / persistent stats capture | ready (needs a short technical-spike decision first) | M | `senior-developer` |
...
remains unused but intentionally kept as a future "easy" difficulty tier.
...
### 4. Multi-profile support — `ready` — size unclear, likely M — agent TBD
Added 2026-07-04 from user suggestions (`SUGGESTIONS.md`): multiple named
profiles on the same device, each with their own achievements/stats (and,
per item 11, presumably their own replay/score history too). Already
anticipated, not yet built: `DONE.md` item 13 (Achievement system) shipped
`SaveStore.resolveSaveFile(String profileName)` specifically so this
wouldn't require a storage-format migration later — today it's always
...
### 9. Round summary panel enhancements — `ready` — S-M — `game-designer` → `senior-developer`
...
**Originally blocked on the AI & polish item** (now `DONE.md`) until multiple
...
place; scoping it now would be premature.
...
design pass that produced `design/ai-and-polish.md`. That doc flags this as
unscoped future work rather than detailing it. Confirmed by the user as out
of scope for that item's v1 pass — logged here as its own item so it isn't
lost, not otherwise scoped or estimated.
...
progression): an entry in item 5's planned extensible dev-settings surface
letting the user swap which AI tier/personality is seated in a game right
now, for testing, plus exposing (for debugging only, not player-facing)
which personality/tier is actually running under the hood per opponent seat.
